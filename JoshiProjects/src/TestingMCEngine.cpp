...
int main()
{
    ...
    ConvergenceTable gathererTwo(gathererMore);
    ...
    for( auto& thisRow : results)
    {
    	for( auto& itm : thisRow)
    	{
    		std::cout<< itm << " ";
    	}
    ...
    char* headers = "Mean, Variat_var, Sum_var, Sum_sd, paths \n";
    char* outfile = "MCConvergenceTable.csv";
    writetable(results,headers, outfile);

//    cout << "MC Call Price " << results[results.size()-1][0] << endl;
//    cout << "MC Variane " << results[results.size()-1][1] << endl;
//    cout << "MC Number of paths " << results[results.size()-1][2] << endl;
