...
int main()
{
	...
	boost::function<void (double)> thisBumpFunc;

	//Get analytic Digi del
	double AnalDigDel = BlackScholesCallTheta(Spot,
												Strike,
												r,
												d,
												Vol,
	...
	StatisticsMean gatherer, gatherer_bump;
	PayOffDigitalCall theLoopPayOff(Strike);
	VanillaOption theLoopOption(theLoopPayOff,Expiry);
	...
	double thisMCprice = gatherer.GetResultsSoFar()[0][0];
//	GenTwo.SetSeed(200);
	SimpleMonteCarlo6(theLoopOption,
							Spot+BumpSize,
							VolParam,
	...
	double thisMCprice_bumped = gatherer_bump.GetResultsSoFar()[0][0];

	cout << "First MC price " << thisMCprice<< endl;
	cout << "bumped MC price " << thisMCprice_bumped << endl;

	double MC_del = (thisMCprice_bumped - thisMCprice)/BumpSize;
	cout << MC_del << endl;
//
//	bool doSim1 = false;
//	//BumpDel Sim
...
//
	...
	//do MC Greek using different numbers
